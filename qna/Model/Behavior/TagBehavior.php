...
class TagBehavior extends ModelBehavior {
    ...
    // Define the new tag model
    	...
    	App::uses('Tag', 'Model');
    	$Tag = new Tag;
        if (!empty($model->data[$model->name][$this->settings[$model->name]['table_label']]) && $Tag->hasField($this->settings[$model->name]['tag_label'])) {
        ...
        foreach($tag_list as $t) {
        	$condition = $this->settings[$model->name]['tag_label'] . " = '" . $t . "'";
            if ($res = $Tag->find('first', array('conditions' => array($condition)))) {
                $tag_info[] = $res['Tag']['id'];
            } else {
